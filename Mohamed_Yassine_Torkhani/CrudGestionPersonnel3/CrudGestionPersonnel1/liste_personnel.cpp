...
int liste_personnel::ajouter_personnel(int cin,QString nom,QString prenom,QString adresse,int age,int tel,QString statut_familiale,QString idposte)
{
    ...
    QString res2=QString::number(age);
        query.prepare("INSERT INTO PERSONNEL (CIN,NOM,PRENOM,ADRESSE,AGE,TEL,STATUT_FAMILIALE,IDPOSTE)"
                      "VALUES (:cin,:nom,:prenom,:adresse,:age,:tel,:statut_familiale,:idposte)");
...
bool liste_personnel::ajouter_personnel(const securite &sec)
{
    ...
    query.bindValue(":formation",sec.get_formation());
